...
function getUserQuizzes() {
  ...
  for (let i = 0; i < quizzIds.length; i++) {
    promise = axios.get(
      `https://mock-api.driven.com.br/api/v6/buzzquizz/quizzes/${quizzIds[i]}`
    );
...
function printNewQuizzButton() {
  ...
  if (userQuizzesList.length === 0) {
    ...
    userQuizzes.innerHTML =
      `<div class="quizzes-title flex align-items-center">
        <h3> Seus Quizzes </h3>
...
function showHomePage() {
  ...
  document.querySelector(
    ".user-quizzes"
  ).innerHTML = `<ul class="user-quizzes-list"></ul>`;
  document.querySelector(
    ".all-quizzes"
  ).innerHTML = `<div class="quizzes-title">
